...
import {
  ...
  updateComment,
  deleteComment,
} from "@/modules/ads/services/ad.service"; // Importar createComment
...
import {
  ...
  UserCircle,
  SendHorizonal,
} from "lucide-react"; // Importar icono de envío
...
const CommentCard: React.FC<{
  ...
  onEdit: () => void;
  onDelete: () => void;
}> = ({ comment, isOwner, onEdit, onDelete }) => {
  return (
    <div className="border-b border-gray-100 p-4 last:border-b-0">
      <div className="flex items-center space-x-3 mb-2">
        ...
        <span className="text-xs text-gray-400">
          {new Date(comment.fechaPublicacion).toLocaleDateString("es-PE", {
            day: "numeric",
            month: "short",
            year: "numeric",
          })}
...
const CommentFormComp: React.FC<CommentFormProps> = ({
  adId,
  onCommentCreated,
}) => {
  const {
...
export function CommentSection({ adId }: CommentSectionProps) {
  ...
  const [commentsResponse, setCommentsResponse] =
    useState<CommentsListResponse | null>(null);
  ...
  /*const handleEdit = async (comment: AdComment) => {
    const isOwner =
      ...
      ((comment.usuario as any)?.id === authUser.id ||
        ...
        comment.usuario?.email === authUser.email);
    if (!isOwner) return;
    const nuevo = prompt("Editar comentario", comment.contenido);
    if (nuevo === null) return;
    if (!nuevo.trim()) return alert("El comentario no puede estar vacío.");
    setActionLoading(true);
    try {
      await updateComment(adId, comment.id, nuevo.trim());
  ...
  */
  ...
  return (
    <div className="mt-10 pt-6 border-t">
      ...
      {successMessage && (
        <div className="mb-3 text-sm text-green-700">{successMessage}</div>
      )}
        ...
        <div className="p-4 bg-yellow-50 rounded-lg mb-6 border border-yellow-200 text-sm text-yellow-800">
          <p>
            ...
            <Link
              ...
              className="font-semibold text-yellow-900 underline hover:no-underline"
            >
              iniciar sesión
            </Link>{" "}
            para poder dejar un comentario.
          </p>
        </div>
      )}
